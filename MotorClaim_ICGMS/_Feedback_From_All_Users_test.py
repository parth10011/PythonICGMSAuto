...
def test_MotorClaim_FeedBackFromCustomer():
    # Login
    setUp()
    time.sleep(1)
...
def test_MotorClaim_FeedbackFromInsurer():
...
def test_MotorClaim_FeedbackFromRepairer():
    setUp()
    time.sleep(1)
...
def test_MotorClaim_FeedbackFromSurveyor():
    setUp()
    time.sleep(1)
...
def test_MotorClaim_FinalCompletionFeedback():
    setUp()
    time.sleep(1)
    ...
    completeClaim_button = wait.until(EC.presence_of_element_located((By.XPATH, "//button[text()='Close']/following-sibling::button[text()='Complete Claim']")))
